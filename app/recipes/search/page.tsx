...
import StarRating from "@/app/[id]/StarRating";
...
export default function Recipe() {
        ...
        <div className="bg-gray-300 rounded-lg p-5 flex-1">
          <div className="flex-1 flex">
            <div className="w-2/3 text-left pr-4">
              ...
              {baseText(
                `材料 ：${materialsText(recipeMap.ingredients.map((element) => element.name))}`
              )}
              {baseText(`作成時間 ：${recipeMap.cooking_time}分`)}
              {baseText(
                `投稿日時 ：${new Date(recipeMap.created_at).toLocaleDateString(
                  "ja-JP",
                  {
                    year: "numeric",
                    month: "2-digit",
          ...
          <div className="items-center mt-3 flex justify-around">
            <div className="flex items-center">
              ...
              >
                {ratingFromReviews(recipeMap.reviews, "would_eat_again")}
              </span>
            </div>
            <div className="flex items-center">
            ...
            <div className="flex items-center">
              ...
              <span
                ...
                style={{ verticalAlign: "middle" }}
              >
                {ratingFromReviews(recipeMap.reviews, "rating")}
              </span>
            </div>
          </div>
        </div>
  ...
  const buttonTable2 = [
    ...
    },
    {
      ...
      name: "材料難易度順",
      onClick: () => {
        sortByIngredientAcquisition(5);
      },
    },
  ];
  // 各種ソート関数
  function sortListByFunction(
    list: any[],
    func: { (user: any): any; (user: any): any; (arg0: any): any }
  ) {
  ...
  function sortByAddedDate(id) {
    const newRecipe = sortListByFunction(
      recipeList,
      (recipe) => -new Date(recipe.created_at).getTime()
    );
  ...
  function sortByRate(id) {
    const newRecipe = sortListByFunction(
      recipeList,
      (recipe) => -ratingFromReviews(recipe.reviews, "rating")
    );
    setresipeList(newRecipe);
    handleClick(id);
  }
  ...
  function sortByWouldYouEatAgain(id) {
    const newRecipe = sortListByFunction(
      recipeList,
      (recipe) => -ratingFromReviews(recipe.reviews, "would_eat_again")
    );
    setresipeList(newRecipe);
    handleClick(id);
  }
  ...
  function sortButton(button, selectedId) {
    return (
      <button
        className={`inline-flex items-center px-4 border text-sm ${button.id === selectedId ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
        ...
        {recipeList && recipeList.length > 0 ? (
          <ul>
            {recipeList.map((recipe, index) => (
              <li key={index}>{recipeCard(recipe)}</li>
            ))}
          </ul>
        ) : (
